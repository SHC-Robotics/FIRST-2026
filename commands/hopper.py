...
class JostleHopper(commands2.Command):
    ...
    def execute(self) -> None:
        ...
        if self.hopperSubsystem.getExtensionPosition() < 0.1 or self.hopperSubsystem.getExtensionPosition() > HopperConstants.EXTENSION_NUM_ROTATIONS:
            self.hopperSubsystem.stop()
...
class WaitJostleHopper(commands2.SequentialCommandGroup):
    def __init__(self, hopperSubsystem: CANHopperSubsystem) -> None:
        super().__init__()
...
class LaunchJostleHopper(commands2.ParallelCommandGroup):
    def __init__(self, fuelSubsystem: CANFuelSubsystem, hopperSubsystem: CANHopperSubsystem) -> None:
        super().__init__()

        self.addCommands(
            WaitJostleHopper(hopperSubsystem),
            Launch(fuelSubsystem)
        )
